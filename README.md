...
- **Tech stack**: Next.js App Router, React, TypeScript, HeroUI, Tailwind CSS, Framer Motion, next-themes. See `package.json`.
...
App Router fits this app because it enables server-side data fetching with caching, clean server/client separation (server fetch + client filtering), and shared layouts/providers without extra wiring. Trade-off: stricter server/client boundaries and a bit more mental overhead than the old Pages Router.

## Accessibility

- Skip link to main content in `app/layout.tsx`.
- All interactive elements are keyboard accessible.
- Results count is announced via `aria-live`.
